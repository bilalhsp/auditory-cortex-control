...
@register_metadata('ucdavisAct')
def build_ucdavisAct():
    return UCDavisActiveMetaData('ucdavisAct', f'configs/ucdavisAct_config.yml')

DATASET_NAME = NEURAL_DATASETS[3]
@register_metadata('ucdavisBAK')
def build_ucdavisBAK():
    return UCDavisActiveMetaData('ucdavisBAK', f'configs/ucdavisBAK_config.yml')



class UCDavisActiveMetaData(BaseMetaData):
    def __init__(self, dataset_name, config_path=None):
        # self.data_dir = DATA_DIR

        self.data_dir = neural_data_dir / dataset_name
        curr_dir = Path(__file__).parent
        stim_super_sets_path = curr_dir / 'stim_super_sets.npz'
        # stim_super_sets.npz contains all stimulus ids (mVocs and TIMIT) for all sessions,
        # this file is saved first time interacting with this dataset and is itself
        # created by reading stimulus ids from each sessions files. This is required because stimulus ids are not
        # consistent with metadata files provided originally.
        stim_super_sets = (
            np.load(stim_super_sets_path)
            if stim_super_sets_path.is_file()
            else None
        )

        self.cfg = RecordingConfig(config_path)

        self.sessions_dict = self.cfg.sessions_dict
        self.sampling_rate = 48000
        # reading timit metadata
        self.timit_meta_file = os.path.join(self.data_dir, 'recanzone_timit_details.mat')
        self.timit_meta = UCDavisActiveMetaData.read_stim_meta(self.timit_meta_file)
        ...
        self.mVocs_meta = UCDavisActiveMetaData.read_stim_meta(self.mVocs_meta_file)
        ...
        self.all_timit_stim_ids, self.all_mVocs_stim_ids = (stim_super_sets['timit_ids'], stim_super_sets['mVocs_ids']) if stim_super_sets is not None else (None, None)
    ...
    def get_all_stim_ids(self, mVocs=False):
        ...
        """
        if mVocs:
            return self.all_mVocs_stim_ids
        else:
            return self.all_timit_stim_ids
    ...
    def get_stim_audio(self, stim_id, mVocs=False):
        ...
        if stim_id not in stim_audios:
            ...
            stim_audio = UCDavisActiveMetaData.read_wfm(stim_filepath)
